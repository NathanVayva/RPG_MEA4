...
void Game::jouer() {
	...
	while (this->hero->getVie() > 0) {
		...
		this->hero->fullDescription();

		this->lireMessages();

		char c = getchar(); // Utilisation de _getch() pour capturer la touche appuyée sans appuyer sur Entrée
		while (this->actions.count(c) == 0) {
			c = getchar(); // Attendre une touche valide
		}
