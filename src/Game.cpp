...
Game::Game(Personnage* hero) : carte(20, hero) {
	...
	actions['u'] = [this](Personnage& hero) {
		// utilise un objet de l'inventaire

		//vector<Consommables> inventaire = this->hero->getInventaire() ;

		// string message = "				INVENTAIRE: \n";

		// for (long unsigned int i=0; i< inventaire.size(); i++)
		// {
		// 	message += to_string(i) + ": " + inventaire[i].getNom() + "\n";
		// }


		this->hero->utiliserObjet( this->hero ); // on utilise un objet sur soi-même
	};

	actions['\n'] = [this](Personnage& hero) {
		// Ne fait rien (attendre un tour)
	};

	actions['m'] = [this](Personnage& hero) {
		// Invoque le magasin

		int seed = 0;
		for (int i = 0; i < this->carte.getSize(); i++) {
			if (this->carte.isEmptyAt(Coordonnee(i, i))) {
				seed += i;			//seed lié à la carte
			}
		}

		this->hero->magasin(seed);
	};

    // Equipements disponibles
    this->equipements.push_back(new Armes("Epée de feu", "e", 100, 15, 5));
    this->equipements.push_back(new Armures("Armure de glace", "a",  120, 5, 15));
    this->equipements.push_back(new Armes("Bâton magique", "b",  120, 5, 20));
    this->equipements.push_back(new Armures("Bouclier robuste", "o",  80, 8, 5));
    this->equipements.push_back(new Consommables("Potion de soin", "s",  20, 15, 0, 0, 0, 0));
    this->equipements.push_back(new Consommables("Elixir de mana", "m",  25, 0, 0, 10, 0, 0));
    this->equipements.push_back(new Consommables("Potion de force", "g",  30, 0, 15, 0, 0, 0));
    this->equipements.push_back(new Consommables("Potion de résistance", "r",  35, 0, 0, 0, 10, 10));

    // Monstres disponibles
    this->monstres.push_back(new ChauveSouris("ChauveSouris",6,2,6,30));
	this->monstres.push_back(new Zombie("Zombie",10,3,10,50));
	this->monstres.push_back(new Vampire("Vampire",12,4,12,70));
	this->monstres.push_back(new Troll("Troll",15,5,15,100));
...
void Game::placerCoffreAleatoire() {
    ...
    Coffre* tresor = new Coffre("Coffre", &objet);
